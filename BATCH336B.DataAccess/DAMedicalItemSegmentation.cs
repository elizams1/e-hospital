...
{
    public class DAMedicalItemSegmentation
    {
        private VMResponse response = new VMResponse();
        private readonly BATCH336BContext db;
        public DAMedicalItemSegmentation(BATCH336BContext _db)
        {
            db = _db;
        }
        ...
        public VMResponse GetByFilter(string filter)
        {
            try
            {
                List<VMMMedicalItemSegmentation> data = (
                    from mic in db.MMedicalItemSegmentations
        ...
        public VMResponse GetById(long id)
        {
            try
            {
                if (id > 0)
                {
                    VMMMedicalItemSegmentation? data = (
                    from mic in db.MMedicalItemSegmentations
                    where mic.IsDelete == false
                        && mic.Id == id
                    select new VMMMedicalItemSegmentation
        ...
        public VMResponse Create(VMMMedicalItemSegmentation data)
        {
            ...
            using (IDbContextTransaction dbTran = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    MMedicalItemSegmentation medItemCategory = new MMedicalItemSegmentation();
        ...
        public VMResponse Update(VMMMedicalItemSegmentation data)
        {
            using (IDbContextTransaction dbTran = db.Database.BeginTransaction())
            {
                try
                {
                    VMMMedicalItemSegmentation? existingData = (VMMMedicalItemSegmentation?)GetById(data.Id).data;
        ...
        public VMResponse Delete(int Id, int userId)
        {
            if (Id != 0 && userId != 0)
            {
                using (IDbContextTransaction dbTran = db.Database.BeginTransaction())
                {
                    try
                    {
                        VMMMedicalItemSegmentation? existingData = (VMMMedicalItemSegmentation?)GetById(Id).data;
                        if (existingData != null)
                        {
                            MMedicalItemSegmentation medItemCategory = new MMedicalItemSegmentation()
                            {
